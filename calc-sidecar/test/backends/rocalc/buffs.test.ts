...
function tkShim() {
  ...
  s.setStats({ str: 80, agi: 90, vit: 40, int: 1, dex: 60, luk: 30 });
  return s;
}

test("taekwon_ranker triples maxHp/maxSp", () => {
  const base = tkShim().readDerivedStats();
  const s = tkShim();
  s.setBuffs([{ name: "taekwon_ranker", level: 1 }]);
  const buffed = s.readDerivedStats();
  assert.ok(
...
test("mild_wind holy endow changes combat damage vs an undead target", () => {
  ...
  const a = tkShim();
  a.setEnemyInline(target);
  const plain = a.readCombatResults().damage.ave;

  const b = tkShim();
  b.setBuffs([{ name: "mild_wind", level: 7, element: "holy" }]);
  b.setEnemyInline(target);
  const endowed = b.readCombatResults().damage.ave;
...
test("unknown buff name throws", () => {
  const s = tkShim();
  assert.throws(() => s.setBuffs([{ name: "not_a_buff" }]), /unknown buff/);
});
...
test("endow element above resolved level throws", () => {
  const s = tkShim();
  assert.throws(
    () => s.setBuffs([{ name: "mild_wind", level: 4, element: "holy" }]),
...
test("spurt mastery (329) drives barehanded combat damage independently of ATK", () => {
  ...
  const base = tkShim();
  base.setEnemyInline(BARE_TARGET);
  const baseDmg = base.readCombatResults().damage;

  // Mastery only via setSkills (bypasses setBuffs to isolate 329 from 379)
  const withMastery = tkShim();
  withMastery.setSkills([{ id: 329, level: 10 }]);
  withMastery.setEnemyInline(BARE_TARGET);
  const masteryDmg = withMastery.readCombatResults().damage;
  ...
  const baseDerived = tkShim().readDerivedStats();
  const masteryDerived = tkShim();
  masteryDerived.setSkills([{ id: 329, level: 10 }]);
  assert.equal(
    masteryDerived.readDerivedStats().atk.base,
...
test("spurt setBuffs applies both status (379) and mastery (329) to combat", () => {
  ...
  const statusOnly = tkShim();
  statusOnly.setSkills([{ id: 379, level: 1 }]);
  statusOnly.setEnemyInline(BARE_TARGET);
  const statusDmg = statusOnly.readCombatResults().damage;

  const both = tkShim();
  both.setBuffs([{ name: "spurt", level: 7 }]);
  both.setEnemyInline(BARE_TARGET);
  const bothDmg = both.readCombatResults().damage;
...
test("mild_wind order: earth at level 1 is accepted (lowest unlock)", () => {
  ...
  const s = tkShim();
  assert.doesNotThrow(
    () => s.setBuffs([{ name: "mild_wind", level: 1, element: "earth" }]),
...
test("mild_wind order: wind at level 1 throws (wind needs level 2)", () => {
  ...
  const s = tkShim();
  assert.throws(
    () => s.setBuffs([{ name: "mild_wind", level: 1, element: "wind" }]),
    /exceeds.*level|not available at level/i,
...
test("mild_wind order: holy at level 7 is accepted (highest unlock)", () => {
  ...
  const s = tkShim();
  assert.doesNotThrow(
    () => s.setBuffs([{ name: "mild_wind", level: 7, element: "holy" }]),
...
test("mild_wind order: holy at level 6 throws (holy needs level 7)", () => {
  ...
  const s = tkShim();
  assert.throws(
    () => s.setBuffs([{ name: "mild_wind", level: 6, element: "holy" }]),
    /exceeds.*level|not available at level/i,
